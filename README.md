# Nginx Stream Dockerfile - taken from tekn0ir/nginx-stream
...
```bash
$ docker build -t="rmtodd618/nginx-stream" github.com/rmtodd618/nginx-stream
```
...
Start deamon
```bash
$ docker run -d -p 0.0.0.0:80:80 --name nginx rmtodd618/nginx-stream
```
...
```bash
$ docker run -d -p 80:80 -p 65432:65432 -v `pwd`\http.conf.d:/opt/nginx/http.conf.d  -v `pwd`\stream.conf.d:/opt/nginx/stream.conf.d --name nginx rmtodd618/nginx-stream
```
